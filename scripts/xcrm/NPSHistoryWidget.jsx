...
export default function NPSHistoryWidget({ responses }) {
  const npsHistory = React.useMemo(() => {
    ...
    responses.forEach(response => {
      if (!response.overall_rating) return;
      ...
      if (response.overall_rating >= 4) monthlyData[monthKey].promoters++;
      if (response.overall_rating < 3) monthlyData[monthKey].detractors++;
    });
